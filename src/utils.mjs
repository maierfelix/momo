...
//import OBJLoader from "webgl-obj-loader";
...
export function readObjectFile(path) {
  ...
  let object = tolw.loadObj(readBinaryFile(path));
  let {tangents} = calculateTangentsAndBitangents(object);
  object = {
  ...
  return object;
  /*let opts = { calcTangentsAndBitangents: true };
  let mesh = new OBJLoader.Mesh(fs.readFileSync(path, "utf-8"), opts);
  let out = {
...
export function readJPEGFile(buffer) {
  return jpegturbo.decode(buffer);
};

export function readPNGFile(buffer) {
  return lodepng.decode(buffer);
};

export function readHDRFile(buffer) {
  let result = parseHDR(buffer);
  let [width, height] = result.shape;
  let {data, exposure, gamma} = result;
  return {
    width,
    height,
    data,
...
export const DEFAULT_TEX_WIDTH = 16;
export const DEFAULT_TEX_HEIGHT = 16;

export function getAbsoluteTextureDimension(textures) {
  ...
  return {
    width: DEFAULT_TEX_WIDTH,
    height: DEFAULT_TEX_HEIGHT
  };
};
...
export function isHDRFile(buffer) {
  let viewU8 = new Uint8Array(buffer);
  let offset = 0x0;
  return (
    viewU8[offset++] === 0x23 &&
