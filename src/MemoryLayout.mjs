export default class MemoryLayout {
  ...
  set(name, value) {
    ...
    if (value.constructor === Number) {
      ...
      let srcView = new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
      dstView.set(srcView, 0x0);
    }
    else if (value.constructor === ArrayBuffer) {
      let srcView = new Uint8Array(value);
      dstView.set(srcView, 0x0);
    }
    else if (value.constructor === BigInt) {
      let srcView = new Uint8Array(new BigInt64Array([value]).buffer);
      dstView.set(srcView, 0x0);
    }
    else {
